...
public:
	...
	void GetFloatData(float *result) const{
		memcpy(result,m[0].GetArray(),4*sizeof(float));
		memcpy(result+4,m[1].GetArray(),4*sizeof(float));
		memcpy(result+8,m[2].GetArray(),4*sizeof(float));
		memcpy(result+12,m[3].GetArray(),4*sizeof(float));
	}
	...
	const Matrix44f & AlignOxToD ( const Vector3f& d);												//added to gxbase Matrix44f
	const Matrix44f & AlignOyToD ( const Vector3f& d);												//added to gxbase Matrix44f
	const Matrix44f & AlignDToOx ( const Vector3f& d);												//added to gxbase Matrix44f
	//receives a vector in xOy plane, d, and aligns it to (1,0,0)
	const Matrix44f & AlignOxToD2D ( const Vector3f& d);											//added to gxbase Matrix44f
	const Matrix44f & MakeRotateAboutAxis ( const Vector3f& d, float cosTheta, float sinTheta);		//added to gxbase Matrix44f
	const Matrix44f & AlignDToV ( const Vector3f& d, const Vector3f& v);							//added to gxbase Matrix44f
...
Matrix44f Matrix44f::operator * ( const Matrix44f &m2 ) const
{
    ...
    for (int i=0; i<4; i++)
        for (int j=0; j<4; j++)
            ...
            R[i][j] = m[i][0]*m2[0][j] + m[i][1]*m2[1][j] +
                      m[i][2]*m2[2][j] + m[i][3]*m2[3][j];
...
inline Vector4f Matrix44f::operator *(const Vector4f& v) const{
	Vector4f result;
	Vector4f row=m[0];
	result[0]=row*v;
	row=m[1]; result[1]=row*v;
	row=m[2]; result[2]=row*v;
	row=m[3]; result[3]=row*v;
	return result;

}
inline Vector4f Matrix44f::operator *(const Vector3f& v) const{
	return (*this)*Vector4f(v.x(),v.y(),v.z(),0.0);
}

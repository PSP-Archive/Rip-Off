...
#include "LaserEffect3DRenderer.h"

#include <vector>
using namespace std;

class LaserEffect3D: public EffectIF {
public:
...
private:
	...
	LaserEffect3DRenderer m_renderer;
};

inline LaserEffect3D::LaserEffect3D():m_pos(0,0,0),
									  m_duration(2)
{
...
inline LaserEffect3D::~LaserEffect3D(){
...
inline void LaserEffect3D::setWidth2(float maxWidth2){
	m_maxWidth2=maxWidth2;
}
inline void LaserEffect3D::setHeight2(float maxHeight2){
	m_maxHeight2=maxHeight2;
}

inline void LaserEffect3D::Start(){
...
inline void LaserEffect3D::Render(){
	if(!isFinished()){
		...
		m_renderer.applyTransforms(m_pos,v,-getRotation(),0.2f+m_maxWidth2,-m_delta,1);
		m_vpIcons[0]->renderGeometry(Vector3f(0,0,0));
		m_renderer.discardTransforms(1);
...
inline void LaserEffect3D::Update(){
	if (!isFinished()){
